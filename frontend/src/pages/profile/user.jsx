          ...
          {uid === id ? (
            <SideContent>
              ...
              {allowEdit ? (
                profilePhoto !== user ? (
                  removePhoto ? (
                    <Label fileselect={fileselect} for='input-file' style={{ marginRight: '10vw' }}>
                      <PublishIcon
                        onClick={(e) => setpublishPhoto(true)}
                        color='primary'
                        style={{ position: 'absolute', marginLeft: '4.5vw', marginTop: '0.7vh', cursor: 'pointer' }}
                      />
                    </Label>
                  ) : (
                    <CancelIcon
                  ...
                  <Label fileselect={fileselect} for='input-file' style={{ marginRight: '10vw' }}>
                    <PublishIcon
                      onClick={(e) => setpublishPhoto(true)}
                      color='primary'
                      style={{ position: 'absolute', marginLeft: '4.2vw', marginTop: '0.7vh', cursor: 'pointer' }}
                    />
                  </Label>
                )
              ) : null}
            ...
            <SideContent>
              ...
              {isfollow === true ? <Button onClick={Tounfollow}>Following</Button> : <Button onClick={Tofollow}>Follow</Button>}
            </SideContent>
          )}
          <Bio>
            ...
            <Tabs>
              <Tab>
                Folios <span>{count}</span>
              </Tab>
              <Tab>
                Reads <span>{read}</span>
              </Tab>
              <Tab>
                    ...
                    <div className='col-xs-12 col-sm-12 col-md-6 col-lg-2'>
                      {uid === id ? (
                        <ProfileTile
                          ...
                          onclick={tileclick}
                          read={p.read_Count}
                          deleted={delete_folio}
                        />
                      ) : (
                        <Tile Title={p.title} Image={p.img_url} id={p.id} onclick={tileclick} Name={p.name} read={p.read_Count} />
                      )}
                    </div>
                  );
                })}
